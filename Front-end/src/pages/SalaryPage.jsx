...
const SalaryPage = () =>{
    ...
    return(
        ...
        {/* salary display card */}
        <div className="grid lg:grid-cols-3 sm:grid grid-cols-1 gap-12 my-12 items-center">
            {
                salary.map((data) =>(
                    <div key={data.id} className="shadow px-4 py-8">
                        <h4 className="font-semibold text-xl">{data.title}</h4>
                        <p className="my-2 font-medium text-blue text-lg">{data.salary}</p>
                        <div><a href="/" className="underline">{data.status}</a></div>
                        <div className="flex flex-wrap gap-4"><a href="/" className="underline">{data.skills}</a></div>
                    </div>
                ))
            }
        </div>
